...
class ResetPasswordWithOTPScreen extends StatefulWidget {
  const ResetPasswordWithOTPScreen({Key? key}) : super(key: key);
...
class _ResetPasswordWithOTPScreenState extends State<ResetPasswordWithOTPScreen>
  ...
  int _currentStep = 0; // 0: Email, 1: New Password, 2: OTP
  ...
  void _handleNextStep() {
    if (_formKey.currentState!.validate()) {
      if (_currentStep == 0) {
        ...
        // التحقق من تطابق كلمات المرور والانتقال لشاشة OTP
          ...
          listener: (context, state) {
            if (state is AppAuthError) {
              ...
              if (state.otpType == OTPType.passwordReset) {
                setState(() {
                  _currentStep = 1; // الانتقال لخطوة إدخال كلمة المرور
                });
              }
            }
          },
  ...
  String _getStepSubtitle() {
    switch (_currentStep) {
      case 0:
        return 'Enter your email to receive verification code';
      case 1:
